(function(){
    ...
    ~function(){
        ...
        utils.css(bannerInner,"width",count*730);
        //2)绑定的是焦点区域的数据
        str="";
        if(jsonData){
            for(i=0,len=jsonData;i<len.length;i++){
        ...
        for(var i=0,len=imgList.length;i<len;i++){
            ~function(i){
                ...
                oImg.onload=function(){
                    ...
                    zhufengAnimate(curImg,{opacity:1},50);
                }
            }(i);
        }
    }
    ...
    function autoMove(){
        ...
        step++;
        zhufengAnimate(bannerInner,{left:-step*730},500);
        changeTip();
    }
    //5.实现焦点对齐
    function changeTip(){
    ...
    bannerLeft.onclick=function(){
        if(step<=0){
            step=count-1;
            utils.css(bannerInner,"left",-step*730);
        }
        step--;
        zhufengAnimate(bannerInner,{left:-step*730},500);
        changeTip();
